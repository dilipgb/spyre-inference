...
Usage::
    ...
    entry = model_registry()["ibm-granite/granite-3.3-8b-instruct"]
    cfg = lookup_config("ibm-granite/granite-3.3-8b-instruct", tp_size=4, max_model_len=32768)
"""
...
class ContinuousBatchingConfig:
    ...
    def from_dict(cls, d: dict[str, Any]) -> ContinuousBatchingConfig:
        raw_dc = d.get("device_config") or {}
        return cls(
            tp_size=d["tp_size"],
            max_model_len=d["max_model_len"],
            max_num_seqs=d["max_num_seqs"],
            device_config=DeviceConfig.from_dict(raw_dc),
        )
...
def lookup_config(
    ...
    """Return the first ``ContinuousBatchingConfig`` matching ``tp_size`` and
    ``max_model_len`` for ``model_id``, or ``None`` if no match is found."""
